...
open class CityDaoTests : XCTestCase {
    ...
    open func test_it_lists_all_cities_alphabetically() {
        let cities = self.cityDao.listAllCities()
        XCTAssertTrue(cities.count > 0)
    }
    
    open func test_it_allows_adding_a_city() {
    ...
    open func test_adding_same_city_twice_does_not_create_duplicates() {
        self.cityDao.saveCity("Manila")
        self.cityDao.saveCity("Manila")
    ...
    open func test_allows_removing_a_city() {
